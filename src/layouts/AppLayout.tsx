...
export function AppLayout() {
  ...
  useEffect(() => {
    progressRef.current = setInterval(
      () => setSplashProgress((p) => Math.min(p + 4, 100)),
      40,
    )
  ...
  return (
    <div
      className={`nature-shell relative flex min-h-screen flex-col select-none ${isHomeRoute ? 'is-home-route' : ''}`}
    >
        ...
        {showSplash && (
          ...
          >
            ...
            <img
              ...
              alt=""
              className="absolute inset-0 h-full w-full object-cover object-center brightness-[0.7] contrast-[1.05] filter"
            />
            ...
            >
              ...
              <div
                aria-hidden="true"
                className="absolute -top-3 -left-3 h-9 w-9 rounded-tr-[70%] rounded-bl-[70%] border-2 border-[#1c380d] bg-gradient-to-br from-[#85c24e] to-[#38661b] shadow-md"
              />
              <div
                aria-hidden="true"
                className="absolute -top-3 -right-3 h-9 w-9 rounded-tl-[70%] rounded-br-[70%] border-2 border-[#1c380d] bg-gradient-to-bl from-[#85c24e] to-[#38661b] shadow-md"
              />

              {/* Top Wooden Ribbon Badge */}
              <div className="absolute -top-4 rounded-full border-2 border-[#3d200e] bg-gradient-to-b from-[#d99043] to-[#8c4b18] px-4 py-0.5 text-[10px] font-black tracking-widest text-[#fff3cd] uppercase shadow-[inset_0_1px_0_rgba(255,255,255,0.6),0_3px_0_#2b1408]">
                <span>SYNIQ MEMORY</span>
              </div>

              {/* Steady 3D Wood Title SYNIQ */}
              <h1
                className="my-3 flex items-center justify-center gap-1 text-3xl font-black tracking-widest uppercase"
                aria-label="SYNIQ"
              >
                {'SYNIQ'.split('').map((char, index) => (
                  <span
                    ...
                    style={{
                      background:
                        'linear-gradient(180deg, #fff3cd 0%, #ffd075 35%, #e59336 65%, #b8621b 100%)',
              ...
              <div className="relative my-2 flex h-9 w-9 items-center justify-center">
                <motion.div
                  ...
                  className="grid h-full w-full grid-cols-2 gap-1 rounded-full border-2 border-[#5a341a] bg-[#e8e0c5] p-1 shadow-[0_3px_0_#3d200e]"
                >
                  <span className="rounded-full border border-[#1c590d] bg-gradient-to-b from-[#64cf4a] to-[#2a8216] shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]" />
                  <span className="rounded-full border border-[#7f1d1d] bg-gradient-to-b from-[#f85b5b] to-[#b91c1c] shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]" />
                  <span className="rounded-full border border-[#0c4a6e] bg-gradient-to-b from-[#38bdf8] to-[#0369a1] shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]" />
                  <span className="rounded-full border border-[#78350f] bg-gradient-to-b from-[#fbbf24] to-[#b45309] shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]" />
                </motion.div>
              </div>

              {/* Loading Bar & Status Text with real splashProgress */}
              <div className="mt-1.5 w-full px-2">
                <div className="mb-1.5 flex justify-between text-[10px] font-black tracking-widest text-[#ffe49e] uppercase drop-shadow-[0_1px_2px_rgba(40,18,6,0.9)]">
      ...
      {showHeader && (
        <header className="nature-header sticky top-0 z-30 w-full transition-all">
          <div className="mx-auto flex w-full max-w-[480px] items-center justify-between px-3.5 py-2">
            {/* Left: User Profile Pill */}
            <button
              type="button"
              onClick={() => setShowAuthModal(true)}
              className="nature-profile group flex cursor-pointer items-center gap-2 rounded-full px-2.5 py-1 text-left transition-colors outline-none"
            >
              <div className="relative">
                <AvatarDisplay
                  avatarId={avatarId}
                  size={28}
                  ringClass="avatar-ring-neon group-hover:scale-105 transition-transform"
                />
                <span className="absolute -right-0.5 -bottom-0.5 h-2 w-2 rounded-full border border-white bg-emerald-400" />
              </div>
              <div className="flex flex-col">
                <span className="max-w-[70px] truncate text-xs font-extrabold text-[#3d4f2d] sm:max-w-[100px]">
                  {playerName}
                </span>
              </div>
            </button>
            ...
            <NavLink
              to="/"
              className="flex items-center gap-1.5 rounded-xl px-2 py-1 transition-colors outline-none"
            >
              <span className="text-sm text-[#d68a19]">✦</span>
              <span className="text-base font-extrabold tracking-widest text-[#587338] uppercase">
                SYNIQ
              </span>
            </NavLink>
      ...
      <main
        id="main-content"
        className="relative z-10 mx-auto w-full max-w-full flex-1 p-0 outline-none sm:max-w-[480px] sm:px-4 sm:py-4"
      >
        <Outlet />
      </main>
